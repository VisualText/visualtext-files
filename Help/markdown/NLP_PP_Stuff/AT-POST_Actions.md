...
| [**singlezap()**](../singlezap.md) | Single-tier reduce. Excise all the nodes in the matched phrase. |
| [**merge()**](../merge.md) | Single-tier reduce that DISSOLVES each top-level node in the matched phrase. |
| [**merger(num1,num2)**](../merger.md) | Single-tier reduce that DISSOLVES each top-level node in the matched range. |
| [**listadd(olist,oitem [, keep])**](../listadd.md) | Add a new node to a list node's children. If the item occurs after the list (***olist*** < ***oitem***), it is added as the last child. If the item occurs before the list, it is added as the first child. The optional keep arg can be "true" or "false". If "true", it keeps the nodes between the list and the item as children of list. If "false", it excises all the intervening nodes. |
| [**excise(num1,num2)**](../excise.md) | Excise the nodes matching the range of elements from the parse tree. For multiple excise actions, operations must be ordered such that the highest number is listed first in the operation. |
| [**splice(num1,num2)**](../splice.md) | DISSOLVE the top level nodes of given range. |
| [**group(num1,num2,label_str)**](../group.md) | Reduce the range of rule elements **(num1,num2)** and name the group node** label**. Unlike other reduce actions, this one can be repeated. ***WARNING:*** The rule element numbers change after each group action, so subsequent POST Actions must account for the new numbering of the rule elements. Reimplemented as an NLP++ function so that args can be NLP++ expressions. |
| [**noop(0)**](../noop.md) | Perform no POST Action. This is a convenience to disable the default **single()** reduce action. |

## Print Actions

| POST Action | Description |
| --- | --- |
| [**print(str)**](../print.md) | Print the literal string **str** to standard output. |
| [**printr(num1,num2)**](../printr.md) | Print the text for the rule element range from **num1** to **num2** to standard output. |
| [**prchild(file, num, name)**](../prchild.md) | Look for **name** immediately under the node matching rule element **num**. Print its text to **file**, if found. |
| [**prtree(file, num, name)**](../prtree.md) | Look for **name** anywhere under the node matching rule element **num**. Print its text to **file**, if found. |
| [**prxtree(filename, prestr,**** ord, name, poststr)**](../prxtree.md) | To the **filename**, print the first node called **name** found in the **ord**th element's tree, preceded by **prestr** and followed by **poststr**. If named node is not found, nothing is printed out. E.g., prxtree("out.txt", "date: ", 3, "_date", "\n") prints out a line like "date: 3/9/99 <cr>" if a _date node is found within the subtree of elt 3. |
| [**prlit(file, str)**](../prlit.md) | Print the literal string **str** to **file**. |
| [**fprintnvar(file, var, ord)**](../fprintnvar.md) | To the **file**, print the value of the variable **var** in the **ord**th element's node. |
| [**fprintxvar(file, var, ord)**](../fprintxvar.md) | To the **file**, print the value of the variable **var** in the **ord**th context node. |
| [**fprintgvar(file, var)**](../fprintgvar.md) | To the **file**, print the value of the global variable **var.** |
| [**gdump(file)**](../gdump.md) | Dump all the global variables and their values to **file**. |
| [**xdump(file, ord)**](../xdump.md) | Dump all the variables in the **ord**th context node and their values to **file**. |
| [**ndump(file, ord)**](../ndump.md) | Dump all the variables in the **ord**th phrase element's node and their values to **file**. |
| [**sdump(file)**](../sdump.md) | Dump all the variables in the suggested node and their values to **file**. |
| [**prrange(file, num1,num2)**](../prrange.md) | Print the text under a range of rule elements (**num1,num2**) to** file**. |
| [**pranchor(file,num1,num2)**](../pranchor.md) | Print a web URL to **file**. Treat the range (**num1,num2**) as a URL. Use the global variable named **Base** to resolve and print complete relative URLs. (a prior pass must find the <base> HTML tag and set **Base** appropriately. NOT yet using the file's URL, so not handling cases where <base> itself is a relative URL.) |
