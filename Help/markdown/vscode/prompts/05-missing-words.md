...
<!-- desc: Reads missing-words.log from the analyzer's output log and, for each genuine word, adds a properly featured entry (part of speech, root, verb/noun features) to en-full.dict and en-full.kbb — keeping both alphabetized with identical headword sets. -->
...
- Add genuine words only. If the list contains obvious noise — typos, fragments, single letters, run-together tokens, or proper nouns — do NOT pollute the dictionary with them. Collect those in a separate `missing-words-skipped.txt` (with a one-line reason each) so I can review them.
